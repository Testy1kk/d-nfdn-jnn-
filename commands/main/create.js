...
module.exports = {
    ...
    execute: async (message, args) => {
        ...
        if (args.length < 1 || args.length > 2) {
            return message.reply('Please provide the correct arguments. Usage: `create (free/premium/boost/basic/cookie/extreme) [fileName]`.');
        }
        ...
        if (['free', 'premium', 'boost', 'basic', 'cookie', 'extreme'].includes(keyword)) {
            try {
                ...
                await sendMessage(message, keyword, fileName);
...
async function sendMessage(message, keyword, fileName) {
